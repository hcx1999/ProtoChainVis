...
function getFileContent(editor: vscode.TextEditor) {
    if (editor) {
...
function separateImportsAndCode(code: string): { imports: string; codeWithoutImports: string } {
...
function createModuleMockCode(imports: string): string {
    if (!imports.trim()) {
        return '';
    }
...
function compileTypeScriptToJS(code: string, fileName: string): { imports: string; compiledCode: string } {
    try {
...
export function getProtoChain(varible: string, editor: vscode.TextEditor | undefined) {
    if (editor === undefined) { return "无活动编辑器"; }
